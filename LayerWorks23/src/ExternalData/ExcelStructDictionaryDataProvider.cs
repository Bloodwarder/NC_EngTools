...
namespace LayerWorks.ExternalData
{
    internal class ExcelStructDictionaryDataProvider<TKey, TValue> : ExcelDictionaryDataProvider<TKey, TValue> where TValue : struct
    {
        ...
        private protected override void CellsImport(Workbook xlwb, Dictionary<TKey, TValue> importeddictionary)
        {
            ...
            {
                labelindex.Add(((Excel.Range)rng[1, i]).Value.ToString(), i);
            }
            // Обрезаем заголовки
        ...
        {
            internal static T Read(Excel.Range rng)
            {
            ...
            internal static void Write(T sourcestruct, Excel.Range targetrange, Dictionary<string, int> indexdictionary)
            {
                ...
                {
                    ((Excel.Range)targetrange.Cells[1, indexdictionary[fi.Name]]).Value = fi.GetValue(sourcestruct);
                }
            }
        }
    }
