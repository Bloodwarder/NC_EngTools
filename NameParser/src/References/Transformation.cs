...
namespace NameClassifiers.References
{
    ...
    {
        [XmlElement(ElementName = nameof(Source), Type = typeof(SectionReference))]
        public SectionReference Source { get; set; }

        [XmlElement(ElementName = nameof(Output), Type = typeof(SectionReference))]
    ...
    public class LayerValidator
    {
        ...
        [XmlElement(Type = typeof(Transformation))]
        ...
        {
            if (Transformations.Any(t => t.Source.Match(layerInfo)))
            {
                var transformation = Transformations.Where(t => t.Source.Match(layerInfo)).Single();
                var reference = transformation.Output;
                switch (reference)
                {
